...
A standalone, browser-based GCode generator under `web/` that runs directly from
`file:///.../web/index.html` with **no server, no build step, no external
dependencies, and no repo changes outside `web/`**. It converts a depth-map PNG
into GRBL-friendly `.nc` toolpath files.
...
3. **Remaining-material tracking is IN scope** (needed for correct multi-pass
   roughing). Arc/line smoothing, time estimates, and the outline-groove pass
   are **OUT of scope** (see Non-Goals).

## File-URL Architecture (constraints, apply everywhere)

- Files: `web/index.html`, `web/styles.css`, `web/app.js`, `web/worker.js`
  source text, `web/README.md`. All local; no CDN, no `import`, no `fetch()`.
...
// One machining pass over the surface.
PassSpec = {
  id: string,
  name: string,
  toolId: string,        // references ToolSpec.id
  direction: "ltr" | "rtl" | "zigzag", // see Raster Generation
  stepoverMm: number | null,   // null → use tool.stepoverMm
  maxStepdownMm: number | null,// null → use tool.maxStepdownMm
  allowanceMm: number,   // stock left above final terrain (>= 0; finishing = 0)
  enabled: boolean,
}
...
- **Init**: `remaining[i] = stockTopMm` for `cut[i]===1`; `+Infinity` (never
  cut) for `cut[i]===0`.
- **Per-pass target surface**:
  ...
  `cut[i]===1`.
- **Multi-sweep depth stepping** (a pass repeats full sweeps until it reaches
  `target`):
...
- **One `.nc` per enabled pass.** Disabled passes are skipped but the
  remaining-material sim should still be seeded correctly (only run enabled
  passes; they mutate `remaining` in order).
- **Filename**: `<imageBase>_<passIndex>_<toolName>.nc` where `imageBase` is the
  ...
  enabled passes, `toolName` sanitized to `[A-Za-z0-9_]`.
...
3. **Mask skip**: pixels with `cut===0` never appear in any cut move.
...
7. **GCode conventions**: generated text contains `M3 S<rpm>`, `G90`, `G21`,
   `G17`, ends with `M5`/`M2`, and contains **no** `M0`.
...
6. **Raster generation + GCode + downloads (single pass, no remaining sim).**
   Deliver: row/span iteration; all three directions; move sequence; GCode
   header/preamble/footer/format; per-pass `.nc` Blob download.
...
7. **Remaining-material simulation across passes.**
   ...
   stamping; passes run in order and mutate `remaining`.
